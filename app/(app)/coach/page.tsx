...
export default function CoachPage() {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      // Mocking past workouts for context
      const pastSummary = "User has logged 3 strength sessions and 2 runs in the last 7 days. Average intensity is moderate.";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                      ...
                      <div className="flex gap-4 mb-2">
                        ...
                        {ex.weight && (
                          <div className="text-sm">
                            ...
                            <span className="font-bold">{ex.weight}</span>
                          </div>
                        )}
                        {ex.durationMinutes && (
                          <div className="text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-80">
        <FeatureSmall icon={BrainCircuit} title="Smart Analysis" desc="Analyzes your past 30 days of data." />
        <FeatureSmall icon={Sparkles} title="Balanced Plans" desc="Optimizes for cardio and strength." />
        <FeatureSmall icon={Timer} title="Time Focused" desc="Routines tailored to your availability." />
      </div>
    </div>
  );
}
...
function FeatureSmall({ icon: Icon, title, desc }: any) {
  return (
    <div className="flex items-start gap-3 p-4">
      <div className="p-2 rounded-lg bg-white shadow-sm text-primary shrink-0">
